  ...
  if (! isolate(req(input$hold_trials))) {
    ...
    res <- res %>% 
      mutate(trial_number = ntrials, 
             inf_color = ifelse(input$samp_mode == "sample", "blue", "red"))
...
graphics <- reactive({
  ...
  if (quantitative_explan_var()) {
    P <- P %>% gf_line(model_output ~ explan, 
            data = current_mod_vals, size = 2, color = "blue", alpha = 0.5)
    if (input$show_ci)
      P <- P %>% gf_errorbar(lower + upper ~ explan,
                             data = current_mod_vals, color = "blue", alpha = 0.1)
    if (ntrials > 0) {
      P <- P %>% gf_jitter(plot_formula(), data = trial_sample, height = 0.05, 
                 width = jitter_width, alpha = 0.5, color = "green") 
      P <- P %>% gf_line(model_output ~ explan,
    ...
    P <- P %>% gf_errorbar(model_output + model_output ~ explan,
                      data = current_mod_vals, size = 2, color = "blue", width = 0.5, alpha = 0.5)
    if (input$show_ci)
      P <- P %>% gf_errorbar(lower + upper ~ explan, 
