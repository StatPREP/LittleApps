...
EXPLAIN <- imageOutput("explain_image") #Use the click attribute to handle the annotations.

DISPLAY <- tabsetPanel(tabPanel("Main graphic", P), 
                       tabPanel("Explain what's what", EXPLAIN))
keepers <- list(
...
<table><tr><td class="control">`r A` `r B` `r N` `r C` `r PV` `r SEED` `r EV` `r CENTER` `r NINETY`  </td><td class="plot">`r DISPLAY`</td></tr></table>
...
output$explain_image <- renderImage({

  list(src = "images/explain-t-statistic.jpg")
  },
  deleteFile = FALSE # important!
)
```
