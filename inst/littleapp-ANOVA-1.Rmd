...
graphics <- reactive({
  ...
  P %>% 
    gf_refine(
      scale_color_manual(
      values = StatPREP_color_scale(explan_levels(), annots = c("model_vals", "raw_vals"))
      )) %>% gf_theme(legend.position = "none")
})
output$plot1 <- renderPlot({graphics()}) 
output$tab <- renderTable("hoo")
```
...
<table><tr><td class="control">`r SEED` `r SAMPN` `r A` `r B` `r SHOWSD`</td><td class="plot">`r P`</td></tr></table>
