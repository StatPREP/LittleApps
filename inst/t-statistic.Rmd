...
response_vars <- c("Pulse", "BMI", "TotChol", "BPSys1", "UrineVol1")
...
<table><tr><td class="control">`r A` `r B` `r N` `r C` `r SEED` `r EV` `r CENTER` `r NINETY`</td><td class="plot">`r P`</td></tr></table>
...
point_alpha <- reactive( {
  0.8 / 2^log10(samp_n()) # a trick?
})
the_color <- reactive( {
  the_formula()[c(1,3)]
})
...
graphics <- reactive( {
  ...
  P <- P %>% gf_errorbar(mn + mn ~ xpos, 
                        data = stats_interval(), color = the_color(), 
                      show.legend = FALSE, width = 3*the_width(), size=2, alpha = .5) 
  if ("Conf. interval" %in% input$layers) {
    P <- P %>%
      gf_errorbar(as.formula(paste("ci_lower + ci_upper ~ ", input$explan)), data = stats_interval(), 
                  color = the_color(), width = 1.3*the_width(), size = 2, show.legend = FALSE)
  }
  
  if ("T" %in% input$layers) {
    P <- P %>% 
      gf_errorbar(low + high ~ midpoint, data = t_interval(), width = 1, show.legend = FALSE)
  }
  set.seed(NULL)
  P
})
