...
#' @param shiny flag indicated whether a shiny-reactive function or a mock-reactive
...
#'
#' @export
react_calc <- function(stmts, shiny=FALSE, env = parent.frame()) {
  stmts <- substitute(stmts)
  if (shiny) {
    F <- reactive(stmts, env = env)
  } else {
    F <- function() { NULL }
    body(F) <- stmts
  }
